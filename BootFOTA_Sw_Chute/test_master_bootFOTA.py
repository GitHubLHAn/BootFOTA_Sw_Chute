...
if __name__ == "__main__":
    ...
    # for i in range(0, len(list_hex_data)):
    #     print(list_hex_data[i])
    time.sleep(1)
    ...
    UDP_SOCKET_DIC = {'udp_socket': udp_socket, 'host': HOST_INPUT, 'port': PORT_INPUT}
    
    ID_master_input = ID_master_start
    rlt = True
    while ID_master_input <= ID_master_start+qty_master-1:
        # Start Booting by FOTA *****************************
        mode_current = request_status_master(ID_master=ID_master_input, UDP_SOCKET=UDP_SOCKET_DIC)
            
        while mode_current == APPLICATION_FW_RUNNING:       # application fw is running on chip
            rlt = run_bootFOTA_Fw_master(ID_master_input, UDP_SOCKET_DIC, retry=5)
            print("\n>>>>>>>>>>>>>> WAIT MCU RESET AND RUNNING BOOT FOTA.... \n") 
            time.sleep(2) 
            mode_current = request_status_master(ID_master=ID_master_input, UDP_SOCKET=UDP_SOCKET_DIC, retry=100)
        ...
        rlt = start_bootFota_process(ID_master_input, UDP_SOCKET_DIC, addr_start_flash, addr_end_flash)
        ...
        rlt = flashing_master_process(ID_master_input, UDP_SOCKET_DIC, list_hex_data)
        ...
        rlt = run_Application_fw_master(ID_master_input, UDP_SOCKET_DIC, addr_start_flash, version_master, MASTER_CHUTE_CIRCUIT)
